inf_name="SILVA_132_LSURef_tax_silva_full_align_trunc.fasta.fa_freq_summary.txt_prob.txt"
...
data_out_filter_1500 <- data_out_filter[intersect(which(data_out_filter[,2]>1000) , which(data_out_filter[,2]<=1500)),]
data_out_filter_2000 <- data_out_filter[intersect(which(data_out_filter[,2]>1500) , which(data_out_filter[,2]<=2000)),]
data_out_filter_2500 <- data_out_filter[intersect(which(data_out_filter[,2]>2000) , which(data_out_filter[,2]<=2500)),]
data_out_filter_3000 <- data_out_filter[intersect(which(data_out_filter[,2]>2500) , which(data_out_filter[,2]<=3000)),]
...
dev.off()
pdf(file=paste(inf_name,"__max_filtered_plot_2000.pdf",sep = ""),width=14)
plot(data_out_filter_2000[,1]~data_out_filter_2000[,2],data_out_filter_2000,type="l")
dev.off()
pdf(file=paste(inf_name,"__max_filtered_plot_2500.pdf",sep = ""),width=14)
plot(data_out_filter_2500[,1]~data_out_filter_2500[,2],data_out_filter_2500,type="l")
dev.off()
pdf(file=paste(inf_name,"__max_filtered_plot_3000.pdf",sep = ""),width=14)
plot(data_out_filter_3000[,1]~data_out_filter_3000[,2],data_out_filter_3000,type="l")
dev.off()
